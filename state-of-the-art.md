...
- **Industrial pieces** researched in [study/industrial-pieces-v1.md](study/industrial-pieces-v1.md) and being built: `rbac` (NIST), `api-keys`, `audit-trail`, `soft-delete`, `verifactu` ([study IX](study/verifactu-v1.md)) and `crud-resource` all ship on `python/fastapi-users`. Pending: `scim-provisioning`, `oidc-login`, `multi-tenancy`, `outbox`, `rate-limit`, integrations.
