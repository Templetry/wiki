...
**Snapshot: 2026-08-06** · maintained as the single up-to-date picture of what exists, where it lives and what state it is in. History lives in [`journal/`](journal/); decisions in [`adr/`](adr/).
...
| [desktop](https://github.com/Templetry/desktop) | Native desktop app (Wails: Go backend embedding the engine, React/TS frontend) | **v0.2.0** — Windows portable exe + NSIS installer |
...
- GitHub sign-in via OAuth device flow (scopes `repo workflow`); repo-first creation: pick a form, fill the manifest-driven dynamic form, preview the render, create the repo on GitHub.
- Repo & project management, multi-catalog support, full template update cycle — drift detection → assisted update → three-way merge — and an in-app updater checking published stable releases.
- Ships as Windows portable exe + NSIS installer (WebView2 runtime).
...
- Engine `verify` in Docker as a first-class CLI step (ADR-0004 fully realized).
- Desktop: settings sync via GitHub, code signing.
